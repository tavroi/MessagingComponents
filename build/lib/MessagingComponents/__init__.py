...
def create_quick_replies(channel: str, text: str, quick_reply_data: dict) -> dict:
    if channel == CHANNEL_NAME_REST:
        qrw = QuickReplyWeb(get_text_formatted_for_different_channels(channel, text))
        ...
        qrfb = QuickReplyFB(get_text_formatted_for_different_channels(channel, text))
        for item in quick_reply_data:
...
def create_carousel(channel, carousel_data):
    if channel == CHANNEL_NAME_REST:
        ...
        for card in carousel_data:
            buttons = []
            for button in card["button"]:
                buttons.append({"type": button["type"], "url": button["url"], "title": button["title"], "webview_height_ratio": "full"})
            crfb.add_element(title=card["title"], subtitle=card["caption"],
                             image_url=card["image_url"],
                             buttons=card["button"], messanger_extensions=True)
...
def create_buttons(channel: str, text: str, buttons_data:dict) -> dict:
    if channel == CHANNEL_NAME_REST:
        ...
        bfb = ButtonTemplateFB(get_text_formatted_for_different_channels(channel, text))
